    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="row gy-4">
                ...
                @foreach ($manageTransport as $key => $item)
                    <div class="col-xxl-4 col-md-6 {{ $key }} searchItems">
                        @php
                            $params = null;
                            if (@$item->params) {
                                foreach ($item->params as $paramVal) {
...
@push('script')
    <script>
        (function($) {
            "use strict";
            var transportData = @json($manageTransport);
            // Function to filter settings based on search query
            function filterSettings(query) {
                let filteredSettings = [];
                for (var key in transportData) {
                    if (transportData.hasOwnProperty(key)) {
                        var setting = transportData[key];
