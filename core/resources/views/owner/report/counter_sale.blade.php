...
@section('panel')
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="card mb-3">
                <div class="card-body">
                    <form action="{{ route('owner.report.sale.counter') }}" method="GET">
                        <div class="row g-3">
                            ...
                            <div class="col-lg-2 col-md-6">
                                <label class="form-label d-block">&nbsp;</label>
                                <a href="{{ route('owner.report.sale.counter') }}" class="btn btn--secondary w-100">
                        ...
                        <div class="mt-3 pt-3 border-top">
                            ...
                            @if(request('date'))
                                <span class="badge badge--primary me-1">
                                    ...
                                    <a href="{{ route('owner.report.sale.counter', array_diff_key(request()->all(), ['date' => ''])) }}" class="text-white ms-1">×</a>
                                </span>
                            @endif
                            @if(request('trip_id'))
                                <span class="badge badge--info me-1">
                                    <i class="las la-route"></i> {{ $trips->where('id', request('trip_id'))->first()->title ?? 'Trip' }}
                                    <a href="{{ route('owner.report.sale.counter', array_diff_key(request()->all(), ['trip_id' => ''])) }}" class="text-white ms-1">×</a>
                                </span>
                            @endif
                            @if(request('status'))
                                <span class="badge badge--warning me-1">
                                    <i class="las la-check-circle"></i> {{ request('status') == '1' ? __('Confirmed') : __('Cancelled') }}
                                    <a href="{{ route('owner.report.sale.counter', array_diff_key(request()->all(), ['status' => ''])) }}" class="text-white ms-1">×</a>
                                </span>
                            @endif
                        </div>
                    @endif
            ...
            <div class="card mb-4">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h5 class="mb-0">@lang('Counter Sales Performance')</h5>
                        ...
                        <table class="table--light style--two table">
                            <thead>
                                <tr>
                                    ...
                                    <th>@lang('Passenger Info')</th>
                                    <th>@lang('Counter / Office')</th>
                                    <th>@lang('Trip')</th>
                                    <th>@lang('Amount')</th>
                                    <th>@lang('Status')</th>
                                </tr>
                            ...
                            <tbody>
                                @forelse($sales as $sale)
                                    <tr>
                                        ...
                                        <td>
                                            {{ $sale->passenger_details['name'] ?? 'N/A' }}
                                            <br>
                                            <small class="text-muted">{{ $sale->passenger_details['mobile_number'] ?? 'N/A' }}</small>
                                        </td>
                                        <td>
                                            @if($sale->counterManager)
                                                {{ $sale->counterManager->name }}
                                                <br>
                                                <small class="text-muted">({{ $sale->counterManager->counter->name }})</small>
                                            @else
                                                <span class="text--warning">@lang('Manual / Unknown')</span>
                                            @endif
                                        </td>
                                        <td>{{ $sale->trip->title }}</td>
                                        <td class="fw-bold">{{ gs('cur_sym') }}{{ getAmount($sale->ticket_count * $sale->price) }}</td>
                                        <td>
                                            @if($sale->status == 1)
                                ...
                                @empty
                                    <tr>
                                        <td class="text-muted text-center" colspan="100%">@lang('No Counter sales found')</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
...
function exportToCSV() {
    ...
    @if(request()->hasAny(['date', 'trip_id', 'status']))
        csv += '# Counter Sales Report - Filtered Results\n';
        @if(request('date'))
    ...
    csv += 'Journey Date,Passenger Name,Mobile,Counter/Office,Trip,Amount,Status\n';
    ...
    @forelse($sales as $sale)
        ...
        csv += '"{{ $sale->passenger_details['name'] ?? 'N/A' }}",';
        csv += '"{{ $sale->passenger_details['mobile_number'] ?? 'N/A' }}",';
        csv += '"{{ $sale->counterManager ? $sale->counterManager->name . ' (' . $sale->counterManager->counter->name . ')' : 'Manual' }}",';
    ...
    let filename = 'bus_counter_sales_' + new Date().toISOString().split('T')[0] + '.csv';
