                            ...
                            <tbody>
                                @forelse($boardingPoints as $point)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.boarding-points.edit', $point->id) }}"
                                                class="btn btn-sm btn-outline--primary">
                                                <i class="las la-edit"></i> @lang('Edit')
                                            </a>
                                            <button type="button"
                                                class="btn btn-sm btn-outline--{{ $point->is_active ? 'warning' : 'success' }}"
                                                onclick="confirmAction('{{ route('admin.boarding-points.status', $point->id) }}')">
...
@push('breadcrumb-plugins')
    <div class="d-flex flex-wrap justify-content-end gap-2 align-items-center">
        <a href="{{ route('admin.boarding-points.create') }}" class="btn btn--primary btn-sm">
