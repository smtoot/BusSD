...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('schedules', function (Blueprint $table) {
            $table->unsignedBigInteger('starting_city_id')->after('route_id')->default(0);
            $table->unsignedBigInteger('destination_city_id')->after('starting_city_id')->default(0);
        });

        // Drop old columns if they exist
        Schema::table('schedules', function (Blueprint $table) {
            if (Schema::hasColumn('schedules', 'starting_point')) {
                $table->dropColumn('starting_point');
            }
            if (Schema::hasColumn('schedules', 'destination_point')) {
    ...
    {
        Schema::table('schedules', function (Blueprint $table) {
            $table->unsignedBigInteger('starting_point')->after('route_id')->default(0);
