...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $val['name'] = authUser()->username;
        $val['description'] = "Payment By Razorpay";
        $val['image'] = siteLogo();
        $val['prefill.name'] = authUser()->owner_name;
        $val['prefill.email'] = authUser()->email;
        $val['prefill.contact'] = authUser()->mobileNumber;
    ...
    public function ipn(Request $request)
    {
        ...
        if ($sig == $request->razorpay_signature && $deposit->status == Status::PAYMENT_INITIATE) {
            ...
            $notify[] = ['error', "Invalid Request"];
            return to_route('owner.deposit.index')->withNotify($notify);
        }
