...
class ForgotPasswordController extends Controller
{
    ...
    public function sendResetCodeEmail(Request $request)
    {
        ...
        $counterManager = CounterManager::where($fieldType, $request->value)->first();
        if (!$counterManager) {
            $notify[] = ['error', 'The account could not be found'];
            return back()->withNotify($notify);
        }

        OwnerPasswordReset::where('email', $counterManager->email)->delete();
        $code = verificationCode(6);
        $ownerPasswordReset             = new OwnerPasswordReset();
        $ownerPasswordReset->email      = $counterManager->email;
        $ownerPasswordReset->token      = $code;
        $ownerPasswordReset->created_at = Carbon::now();
        $ownerPasswordReset->save();

        $ownerBrowser = osBrowser();
        notify($counterManager, 'PASS_RESET_CODE', [
            'code' => $code,
    ...
    public function verifyCode(Request $request)
    {
        ...
        return to_route('manager.password.reset', $code)->withNotify($notify);
    }
