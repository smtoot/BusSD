...
trait SupportTicketManager
{
    ...
    public function supportTicket()
    {
        ...
        $supports = SupportTicket::where($this->column, $user->id)->orderByDesc('id')->paginate(getPaginate());
        if ($this->userType == 'owner') {
            return view('owner.support.tickets', compact('supports', 'pageTitle'));
        }
    ...
    public function openSupportTicket()
    {
        ...
        if ($this->userType == 'owner') {
            return view('owner.support.create', compact('pageTitle', 'user'));
        }
    ...
    public function viewTicket($ticket)
    {
        ...
        $messages = SupportMessage::where('support_ticket_id', $myTicket->id)->with('ticket', 'admin', 'attachments')->orderByDesc('id')->get();
        if ($this->userType == 'owner') {
            return view('owner.support.view', compact('myTicket', 'messages', 'pageTitle', 'user', 'layout'));
        }
    ...
    public function replyTicket(Request $request, $id)
    {
        ...
        if (($this->userType == 'owner') && ($userId != $ticket->owner_id)) {
            abort(404);
        }
        $message = new SupportMessage();
